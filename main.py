...
#
    ...
    for k in range(0,271,5):
        ...
        lcd.line(center_point_x,center_point_y,edge_point_x,edge_point_y,colour(255,0,0))
        lcd.line(center_point_x+2,center_point_y,edge_point_x,edge_point_y,colour(255,0,0))
        lcd.line(center_point_x-2,center_point_y,edge_point_x,edge_point_y,colour(255,0,0))

        #clearing needle
        needle_x_coords = [center_point_x,center_point_x+2,center_point_x-2,edge_point_x]
        needle_y_coords = [center_point_y,center_point_y+2,center_point_y-2,edge_point_y]
        blank_rectangle_min_x = min(needle_x_coords)
        blank_rectangle_max_x = max(needle_x_coords)
        blank_rectangle_min_y = min(needle_y_coords)
        blank_rectangle_max_y = max(needle_y_coords)
        blank_rectangle_x_len = blank_rectangle_max_x - blank_rectangle_min_x
        blank_rectangle_y_len = blank_rectangle_max_y - blank_rectangle_min_y
        
        string_value = str(value)
        print(int(string_value[0]))
        if value < 10:
            
            lcd.seg(115,160,int(string_value[0]),6,colour(0,0,0),colour(0,0,0)) #blank segment
            lcd.seg(135,160,int(string_value[0]),6,colour(0,0,0),colour(255,0,0))
            #dot is not a valid digit
            lcd.seg(185,160,int(string_value[2]),6,colour(0,0,0),colour(255,0,0))
        else:
            lcd.seg(95,160,int(string_value[0]),6,colour(0,0,0),colour(255,0,0))
            lcd.seg(135,160,int(string_value[1]),6,colour(0,0,0),colour(255,0,0))
            #dot is not a valid digit
            lcd.seg(185,160,int(string_value[3]),6,colour(0,0,0),colour(255,0,0))
        lcd.fill_rect(170,205,10,10,colour(255,0,0))
        lcd.show()
        lcd.fill_rect(blank_rectangle_min_x,blank_rectangle_min_y,blank_rectangle_x_len+1,blank_rectangle_y_len+1,colour(0,0,0))
    if voltage:
